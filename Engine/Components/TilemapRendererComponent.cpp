...
namespace ChiefEngine {
	...
	void TilemapRendererComponent::Start() {
		m_tilemap = Resources().Get<Tilemap>(m_tilemapName, G_Engine().GetRenderer());
	...
	void TilemapRendererComponent::Draw(const class Renderer& renderer) const {
		// iterate through layers 
		for (auto layer : m_tilemap->GetLayers()) {
	...
	void TilemapRendererComponent::Read(const JSON::value_t& value) {
		...
		JSON_READ_MEMBER_REQUIRED(value, "tilemapName", m_tilemapName);
	}
}